...
def taiwan(h: float, distance: float = 0) -> Polygon:
    """
...
def regularPolygon(nSides: int, radius: float, thetaStart: float = 0, thetaEnd: float = 360) -> Polygon:
    ...
    def _polygon(a, end):
        return tuple(
            (
                radius * cos(radians(thetaStart + i * a)), 
                radius * sin(radians(thetaStart + i * a))
            ) 
            for i in range(end)
        )
...
def star(outerRadius: float = 1, innerRadius: float =  0.381966, n: int = 5) -> Polygon:
    """
    Create a star.
...
def hull2D(points: Iterable[VectorLike]) -> Polygon:
    """
